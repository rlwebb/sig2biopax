...
public class Line {
    ...
    public static String BioSourceDB = "NEWT";
    ...
    public static void ReadLine() throws IOException, NullPointerException
    {
    	try {
			...
			s = new Scanner(input);
			  ...
			  if (inputTemplate.equalsIgnoreCase("lemischka"))
			  {
				  SourceName = s.next();
			  ...
			  else if (inputTemplate.equalsIgnoreCase("chea"))
			  {
				  SourceName = s.next();
				  TargetName = s.next();
				  PubMedID = s.next();
	...
	//			if (s.hasNext()) { throw LongLineException e }
			...
			ResetNAToBlank();
			System.out.print("Processing Line number " + lineCount + "\r");
		}
